...
document.getElementById('surpriseBtn').addEventListener('click', () => {
    ...
    if (name === '') {
        messageArea.innerHTML = `<div class="special-message">Please enter a name first! 💝</div>`;
        return;
    }

    // Function to format message for mobile
    const formatMessage = (text) => {
        return text.replace(/\n/g, ' ').replace(/\s+/g, ' ').trim();
    };

    // Easter egg for Zayd
    if (name.toLowerCase() === 'zayd') {
        messageArea.innerHTML = `
            <div class="special-message">
                <div class="easter-egg-badge">🎮 Ultimate Easter Egg Unlocked! 🎮</div>
                ${formatMessage(`You typed Zayd? Well, congrats on unlocking the most epic prank of all.
                I've been rejected 69 times in a row, but hey, at least I'm consistent.
                Oh, and just a heads-up—I still don't have a Valentine this year...
                but who needs love when you have endless code, right?
                -The Forever Alone Coder 🤓`)}
            </div>`;
        return;
    }

    // Get a random message from the template
    const randomMessage = funnyMessages[Math.floor(Math.random() * funnyMessages.length)];
    const personalizedMessage = randomMessage.replace(/{name}/g, name);
    
    messageArea.innerHTML = `
        <div class="special-message">
            ${formatMessage(personalizedMessage)}
        </div>`;
